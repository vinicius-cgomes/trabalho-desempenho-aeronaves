...
T0 = 288.15; 
T_ajustada = T0 + 20;
rho = 1.225 * (1 - (0.0065 * H0) / T_ajustada)^(g / (R * 0.0065));
...
% Calcular CLde máximo alcance e máximo autonomia
[max_alcance, idx_alcance] = max(results(:,6));
CL_max_alcance = results(idx_alcance, 2);  % CL correspondente ao máximo alcance
disp('   ');
...
CL_max_autonomia = results(idx_autonomia, 2);  % CL correspondente ao máximo alcance
disp('  ');
