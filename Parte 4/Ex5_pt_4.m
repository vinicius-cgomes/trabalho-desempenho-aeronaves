...
% Modelo da aeronave - talvez tenham infos repetidas de outros códigos
...
[~,~,~,rhoCruise,~,~] = atmosisa(Hcruise); % Não tenho ctz se é assim ou se pega da questão 1
[~,~,~,rhoAvg,~,~] = atmosisa(Havg); % Não tenho ctz se é assim ou se pega da questão 1
[~,~,~,rho0,~,~] = atmosisa(0); % Não tenho ctz se é assim ou se pega da questão 1
g = 9.8; % Gravidade [m/s²] add mais casas decimais

deltaT = 1; % Posição da manete
Tmax_SL = 55600; % Tração máxima [N]; AEO; manete = 100%
Tavg = deltaT * Tmax_SL * (rhoAvg/rho0)^0.6; % Modelo de tração; pelo enunciado, entendi que era pra considerar rhoAvg (mas na prática, rho varia com H)
...
xDesc = Hcruise/abs(tan(gamaDesc)); % valor parece alto, será que tá certo? [m]
...
for i=1:3
    ...
    timeStep = 1*60; % Time step; a cada min recalcula a velocidade de cruzeiro
    ...
    legend('Velocidade de em função do tempo', 'Velocidade média', 'Location', 'best');

    % % Calcular o combustível de cruzeiro. No relatório, comentar a
    % % simplificação adotada: estamos considerando a velocidade média de
    % % cruzeiro para calcular o combustível gasto.

    W_endCruise = W_startCruise * exp(-xCruise(i)*TSFC/(vCruiseAvg(i)*CLCDmax)); % Peso do avião no final do cuzeiro [N]
